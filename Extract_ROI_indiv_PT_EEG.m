...
extract_diff = [];
extract_diff = tf_all_Diff_PTMinColor(:,idx_time_used,:,:);

% PT vs. Color
extract_ROI_indiv_PTvsColor = squeeze(squeeze(mean(mean(mean(extract_diff(ROI_freq_range_idx(1):ROI_freq_range_idx(2),ROI_time_range_idx(1):ROI_time_range_idx(2),right_temporal_parietal_idx,:),1), 2),3)));

extract_diff = [];
extract_diff = tf_all_Norm_PT(:,idx_time_used,:,:);
% Normalized PT
extract_ROI_indiv_NormPT = squeeze(squeeze(mean(mean(mean(extract_diff(ROI_freq_range_idx(1):ROI_freq_range_idx(2),ROI_time_range_idx(1):ROI_time_range_idx(2),right_temporal_parietal_idx,:),1), 2),3)));

extract_diff = [];
extract_diff = tf_all_Norm_Color(:,idx_time_used,:,:);
% Normalized Color
extract_ROI_indiv_NormColor = squeeze(squeeze(mean(mean(mean(extract_diff(ROI_freq_range_idx(1):ROI_freq_range_idx(2),ROI_time_range_idx(1):ROI_time_range_idx(2),right_temporal_parietal_idx,:),1), 2),3)));


save([Export_data 'ROI_results'],'extract_ROI_indiv_PTvsColor', 'extract_ROI_indiv_NormPT');