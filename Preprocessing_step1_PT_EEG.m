...
% This is an EEG data preprocessing script based on the MADE pipeline (Debnath et al., 2020).
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for s=1:length(subject_list)
    ...
    EEG = pop_importevent( EEG, 'event',[event_location subject '_OnsetOnly.txt'],'fields',{'latency' 'type'},'timeunit',0.001,'align',0);
    [ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
    
    % Adjust video markers for task related offset
    for j=1:length(EEG.event)
        EEG.event(j).latency=EEG.event(j).latency+(TASK_TF/1000)*EEG.srate;
    end
    
    % Read in interference markers
    EEG = eeg_checkset( EEG );
    EEG = pop_importevent( EEG, 'event',[event_location subject '_OnsetOffset.txt'],'fields',{'latency' 'duration' 'type'}, 'timeunit',0.001,'align',0);
    [ALLEEG EEG] = eeg_store(ALLEEG, EEG, CURRENTSET);
    
    % Adjust EGI anti-aliasing time offset
    for j=1:length(EEG.event)
        EEG.event(j).latency=EEG.event(j).latency+(EGI_AATF/1000)*EEG.srate;
    end
