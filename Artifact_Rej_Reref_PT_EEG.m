...
if strcmp(interp_chan, 'yes')==1
    ...
    %% If more than 10% of channels in an epoch were interpolated in the epoch, reject that epoch
    % Find total number of channels
    numchan=EEG.nbchan;
    i=1;
    badepoch=[];
    for ei = 1:numEpochs
        % Find how many channels are bad in an epoch
...
Exclusion_Info.(subject).(trial_type{trl_type}).Trials_After_Rej = Trials_After_Rej;
